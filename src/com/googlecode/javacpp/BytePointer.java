...
/**
 * The peer class to native pointers and arrays of {@code signed char},
 * including strings. All operations take into account the position and limit,
 * when appropriate.
 *
 * @author Samuel Audet
 */
public class BytePointer extends Pointer {
    ...
    /**
     * Allocates enough memory for the encoded string and actually encodes it in
     * the named charset before copying it.
     *
    ...
    /**
     * Allocates enough memory for the encoded string and actually encodes it in
     * the platform's default charset before copying it.
     *
     * @param s the String to encode and copy
    ...
    /**
     * For direct buffers, calls {@link Pointer#Pointer(Buffer)}, while for
     * buffers backed with an array, allocates enough memory for the array and
     * copies it.
     *
    ...
    public BytePointer(int size) {
        try {
            ...
            System.err.println("No native JavaCPP library in memory. (Has Loader.load() been called?) " + e.getMessage());
        }
    }
    ...
    /**
     * Decodes the native bytes assuming they are encoded in the platform's
     * default charset.
     *
     * @return a String from the null-terminated string
     */
